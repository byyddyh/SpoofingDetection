...
import static cn.byyddyh.spoofingdetection.FileLogger.MeasurementProviderTAG;
...
public class RealTimePositionVelocityCalculator {
    ...
    public PseudorangePositionVelocityFromRealTimeEvents
    ...
    private LogFragment.UIFragmentComponent uiResultComponent;

    public synchronized LogFragment.UIFragmentComponent getUiResultComponent() {
        return uiResultComponent;
    }

    public synchronized void setUiResultComponent(LogFragment.UIFragmentComponent value) {
        uiResultComponent = value;
    }

    /**
     * Update the reference location in {@link PseudorangePositionVelocityFromRealTimeEvents} if the
     * received location is a network location. Otherwise, update the {@link } to
                    ...
                    new Runnable() {
                        ...
                        public void run() {
                            ...
                            try {
                                ...
                                Log.e(MeasurementProviderTAG, " Exception setting reference location : ", e);
                            }
                        }
                    };
...
//                                                                event.getClock().getTimeNanos()));
    ...
    private void logEvent(String tag, String message, int color) {
        String composedTag = MeasurementProviderTAG + tag;
        Log.d(composedTag, message);
        logText(tag, message, color);
    }

    private void logText(String tag, String text, int color) {
        LogFragment.UIFragmentComponent component = getUiResultComponent();
        if (component != null) {
